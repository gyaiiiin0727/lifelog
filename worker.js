...
async function getAuthUser(request, env) {
  ...
  // 2) URLパラメータ（sendBeacon用：ヘッダーを送れない場合のフォールバック）
  if (!token) {
    try {
...
async function handleSyncUpload(body, env, user) {
  ...
  // プランレベル: クライアントから"free"のみ許可（有料プランはStripe経由のみ）
  if (planLevel === "free") {
    const userRaw = await env.SYNC_KV.get(`user:${user.sub}`);
    if (userRaw) {
      const userData = JSON.parse(userRaw);
      // 現在proでないときのみ（Stripe管理のproは上書きしない）
      if (userData.planLevel !== 'pro' && userData.planLevel !== 'premium') {
  ...
  // === 空データ上書き防止 ===
  // ローカルが空なのにクラウドにデータがある場合は拒否
  let localActCount = 0, localJournalCount = 0, localMoneyCount = 0;
  try { localActCount = data.activities ? JSON.parse(data.activities).length : 0; } catch(e) {}
  try { localJournalCount = data.journalEntriesV3 ? Object.keys(JSON.parse(data.journalEntriesV3)).length : 0; } catch(e) {}
  try { localMoneyCount = data.moneyRecords ? JSON.parse(data.moneyRecords).length : 0; } catch(e) {}
  const localTotal = localActCount + localJournalCount + localMoneyCount;

  const existing = await env.SYNC_KV.get(`data:${user.sub}`);

  if (existing && localTotal === 0) {
    try {
      const ep = JSON.parse(existing);
      const ed = ep.data || {};
      let ea = 0, ej = 0, em = 0;
      try { ea = ed.activities ? JSON.parse(ed.activities).length : 0; } catch(e) {}
      try { ej = ed.journalEntriesV3 ? Object.keys(JSON.parse(ed.journalEntriesV3)).length : 0; } catch(e) {}
      try { em = ed.moneyRecords ? JSON.parse(ed.moneyRecords).length : 0; } catch(e) {}
      if (ea + ej + em > 5) {
        // クラウドにデータがあるのにローカルが空 → 上書き拒否
        return json({ error: "empty_override", message: "ローカルデータが空のため、クラウドバックアップを保護しました。" }, 409);
      }
    } catch(e) {}
  }
  ...
  if (existing) {
    try {
      const ep = JSON.parse(existing);
      const historyKey = `data:${user.sub}:history`;
      const historyRaw = await env.SYNC_KV.get(historyKey);
      const history = historyRaw ? JSON.parse(historyRaw) : [];

      // ローテーション: 0〜6のスロットに順番に書き込む
      const slot = history.length % 7;
      const vKey = `data:${user.sub}:v:${slot}`;
      await env.SYNC_KV.put(vKey, existing, { expirationTtl: 30 * 24 * 60 * 60 }); // 30日保持

      history.unshift({ syncedAt: ep.syncedAt, key: vKey });
      if (history.length > 7) history.splice(7);
      await env.SYNC_KV.put(historyKey, JSON.stringify(history));
    } catch(e) {
      // 履歴保存失敗はメインの保存を妨げない
    }
  }
  ...
  await env.SYNC_KV.put(`data:${user.sub}`, JSON.stringify({ data, syncedAt }));

  return json({ ok: true, syncedAt });
}

// GET /api/sync/history
async function handleSyncHistory(env, user) {
  const historyRaw = await env.SYNC_KV.get(`data:${user.sub}:history`);
  const history = historyRaw ? JSON.parse(historyRaw) : [];

  const versions = [];
  for (const item of history) {
    try {
      const raw = await env.SYNC_KV.get(item.key);
      if (raw) {
        const parsed = JSON.parse(raw);
        const d = parsed.data || {};
        let a = 0, j = 0, m = 0;
        try { a = d.activities ? JSON.parse(d.activities).length : 0; } catch(e) {}
        try { j = d.journalEntriesV3 ? Object.keys(JSON.parse(d.journalEntriesV3)).length : 0; } catch(e) {}
        try { m = d.moneyRecords ? JSON.parse(d.moneyRecords).length : 0; } catch(e) {}
        versions.push({ syncedAt: item.syncedAt, key: item.key, actCount: a, journalCount: j, moneyCount: m });
      }
    } catch(e) {}
  }
...
// POST /api/sync/restore-version
async function handleSyncRestoreVersion(body, env, user) {
  const { key } = body;
  if (!key) return json({ error: "バージョンキーが必要です" }, 400);
  if (!key.startsWith(`data:${user.sub}:`)) return json({ error: "アクセス権限がありません" }, 403);

  const raw = await env.SYNC_KV.get(key);
  if (!raw) return json({ error: "バージョンが見つかりません" }, 404);

  const parsed = JSON.parse(raw);
  const syncedAt = new Date().toISOString();

  // このバージョンを最新として復元
  await env.SYNC_KV.put(`data:${user.sub}`, JSON.stringify({ data: parsed.data, syncedAt }));

  return json({ ok: true, data: parsed.data, syncedAt, originalSyncedAt: parsed.syncedAt });
}

// GET /api/sync/download
async function handleSyncDownload(env, user) {
  // ユーザーレコードからplanLevelを先に取得（dataがなくても返す）
  let planLevel = null;
  try {
    const userRaw = await env.SYNC_KV.get(`user:${user.sub}`);
    if (userRaw) {
      const userData = JSON.parse(userRaw);
      planLevel = userData.planLevel || null;
    }
  } catch(e) {}

  const stored = await env.SYNC_KV.get(`data:${user.sub}`);
  if (!stored) return json({ data: null, syncedAt: null, planLevel });

  const parsed = JSON.parse(stored);
  return json({ data: parsed.data, syncedAt: parsed.syncedAt, planLevel });
}
...
async function handleSyncStatus(env, user) {
  ...
  if (!stored) return json({ syncedAt: null, planLevel });

  const parsed = JSON.parse(stored);
  return json({ syncedAt: parsed.syncedAt, planLevel });
}
...
// 初月特典: consult 5回、goalCoach 3回制限
const FIRST_MONTH_LIMITS = { journal: 9999, consult: 5, goalCoach: 3 };
...
// --- Admin helpers (早期定義: handleActiveUsers から参照) ---
const ADMIN_EMAILS_EARLY = ["sugaishu1412@gmail.com"];
function isAdminEmail(email) { return ADMIN_EMAILS_EARLY.includes(email); }

// --- アクティビティ記録 ---
async function recordActivity(env, email) {
  try {
    const now = new Date();
    const dateStr = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
    // 最終アクティブ日時（永続保持）
    await env.SYNC_KV.put(`lastActive:${email}`, now.toISOString());
    // DAU追跡キー（7日間TTL）
    await env.SYNC_KV.put(`dau:${dateStr}:${email}`, "1", { expirationTtl: 7 * 86400 });
  } catch(e) {
    // アクティビティ記録失敗はメイン処理を妨げない
  }
}

// --- アクティブユーザー統計（管理者用）---
async function handleActiveUsers(env, user) {
  if (!isAdminEmail(user.sub)) return json({ error: "管理者権限が必要です" }, 403);
  ...
  // DAU（今日）
  const todayResult = await env.SYNC_KV.list({ prefix: `dau:${today}:` });
  const dau = todayResult.keys.length;
  const dauUsers = todayResult.keys.map(k => k.name.replace(`dau:${today}:`, ""));
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const result = await env.SYNC_KV.list({ prefix: `dau:${toDateStr(d)}:` });
    result.keys.forEach(k => weekUsers.add(k.name.split(":").slice(2).join(":")));
  }
  ...
  for (let i = 0; i < 30; i++) {
    ...
    d.setDate(d.getDate() - i);
    const result = await env.SYNC_KV.list({ prefix: `dau:${toDateStr(d)}:` });
    result.keys.forEach(k => monthUsers.add(k.name.split(":").slice(2).join(":")));
  }
  const mau = monthUsers.size;

  // 全ユーザーの最終アクティブ日時一覧
  const lastActiveResult = await env.SYNC_KV.list({ prefix: "lastActive:" });
  const userActivity = [];
  for (const k of lastActiveResult.keys) {
    const email = k.name.replace("lastActive:", "");
    const lastActive = await env.SYNC_KV.get(k.name);
    userActivity.push({ email, lastActive });
  }
  userActivity.sort((a, b) => (b.lastActive || "").localeCompare(a.lastActive || ""));
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const result = await env.SYNC_KV.list({ prefix: `dau:${dateStr}:` });
    dailyDau.push({ date: dateStr, count: result.keys.length });
  }

  return json({ dau, wau, mau, dauUsers, weekUsers: Array.from(weekUsers), userActivity, dailyDau });
}
...
async function checkAndIncrementUsage(env, email, aiType) {
  ...
  if (limit !== 9999 && current >= limit) {
    return { allowed: false, current, limit, plan, firstMonth };
  }
  ...
  if (plan === "free" && !firstMonth && aiType === "journal") {
    ...
    if (dailyUsed) {
      return { allowed: false, current, limit, plan, firstMonth, dailyLimit: true };
    }
...
async function handleAnalyze(body, env, userEmail, clientIP) {
  const { text, tone, type, isFirstTurn } = body;
  ...
  // goalCoachはセッション最初のターンのみカウント（1セッション=1回）
  const skipUsageCount = (aiType === "goalCoach" && isFirstTurn === false);
...
ユーザーの相談に対して、上記キャラクターになりきり、その口調で回答してください。

★最重要★ これはチャット形式の会話です:
- 1回の返答は2〜3文（80〜150文字）を目安に短く返してください。長文は禁止です。
- 友達とLINEで話すような自然な会話テンポで。
- 一度に全部言わず、相手の反応を見ながら話を進めてください。
- 返答の最後に質問や話題の投げかけを入れて、会話を続けてください。
- 箇条書き・見出し・番号リストは使わないでください。話し言葉だけで。

質問タイプの判別:
【事実質問の場合】（例: 「〜にいくらつかった？」「合計いくら？」）
- データから読み取れる事実を簡潔に回答（1〜2文）
- 数字を伝えた後、キャラクターとして一言添える

【相談の場合】（例: 「〜どうしたらいい？」「悩んでる」）
- まず共感や受け止めを1文
- データに基づいた指摘を1文
- 最後に質問を投げかけて会話を続ける

共通ルール:
- ユーザーの記録データがある場合、具体的な数字に言及する（例: 「食費¥15,000のうち外食60%だね」）
- 一般論ではなく実際のデータに基づいた回答を
- データにないことは「記録からはわからない」と正直に言う
- ★とにかく短く。1回で完結させず、会話で深掘りしていくスタイルで。`;
  } else {
    // ジャーナル: JSON構造化応答
    system = `${charPrompt}
...
出力は必ずJSONのみ（コードブロック禁止）。形式:
{
  ...
  "concerns": "もやっとしたこと・気がかり・不安・心配事（箇条書き）",
  "must": "明日の必須タスク（箇条書き）",
...
該当がなければ空文字。
※重要: "concerns"にはネガティブな気がかり・不安・モヤモヤだけを入れてください。感謝やポジティブな感情は"events"に入れてください。
...
// --- アンケート保存 ---
async function handleSurvey(body, env, user) {
  const answers = body.answers;
  if (!answers || typeof answers !== "object") {
    return json({ error: "回答データが不正です" }, 400);
  }
  ...
  // 集計用: 個別KVにも保存（後からリスト取得可能）
  await env.SYNC_KV.put(`survey:${user.sub}`, JSON.stringify({
    email: user.sub,
    ...answers,
    answeredAt: new Date().toISOString(),
  }), { expirationTtl: 365 * 24 * 60 * 60 }); // 1年保持
...
async function handleCommunitySetMe(body, env, user) {
  ...
  const raw = await env.SYNC_KV.get(`user:${user.sub}`);
  if (!raw) return json({ error: "ユーザーが見つかりません" }, 404);
  const data = JSON.parse(raw);
...
async function handleCommunityCreateArticle(body, env, user) {
  ...
  // 画像（最大3枚、各300KB以下のBase64）
  const images = Array.isArray(body.images) ? body.images.slice(0, 3).filter(s => typeof s === "string" && s.startsWith("data:image/") && s.length < 300000) : [];

  const article = {
    id: articleId, author: user.sub, nickname, title, body: text,
    tags, images: images.length > 0 ? images : undefined,
    type: isAdmin(user.sub) ? "admin" : "user",
...
async function handleCommunityDraft(body, env, user) {
  const answers = body.answers || {};
  const fields = [
    { key: "trigger", label: "きっかけ" },
    { key: "problem", label: "以前の悩み" },
    { key: "usage", label: "使い方" },
    { key: "change", label: "変化" },
  ...
  for (const f of fields) {
    ...
    if (v) userInput += `【${f.label}】\n${v}\n\n`;
  }
  if (!userInput.trim()) return json({ error: "少なくとも1つの項目に入力してください" }, 400);

  const ok = await communityRateLimit(env, user.sub, "article");
  if (!ok) return json({ error: "投稿間隔を空けてください" }, 429);

  const system = `あなたはライフログアプリ「Dayce」のコミュニティ記事作成AIアシスタントです。
ユーザーが各質問に答えた内容を元に、読みやすいコミュニティ記事を作成してください。

ルール:
- ユーザーの回答をもとに、自然で読みやすい記事にまとめる
- 回答がない項目はスキップ
- 段落を分けて読みやすく
- 敬体（です・ます調）で書く
- タイトルは30文字以内で魅力的に
...
必ず以下のJSON形式で返してください:
{
  ...
  const data = await upstream.json();
  if (!upstream.ok) return json({ error: data?.error?.message || "AI処理エラー" }, 500);
  ...
  try {
    ...
    return json({
      ...
      body: String(result.body || "").slice(0, 5000),
      tags: Array.isArray(result.tags) ? result.tags.slice(0, 5).map(t => String(t).trim().slice(0, 20)) : [],
    });
  } catch (e) {
    return json({ error: "AIの応答を解析できませんでした" }, 500);
  }
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/survey" && request.method === "POST") {
        ...
        if (!user) return json({ error: "ログインが必要です" }, 401);
        const body = await request.json();
        return handleSurvey(body, env, user);
      }
      ...
      if (path.startsWith("/api/community/")) {
        ...
        if (path === "/api/community/draft" && request.method === "POST") {
          const body = await request.json();
          return handleCommunityDraft(body, env, user);
        }
      ...
      if (path.startsWith("/api/sync/")) {
        ...
        if (path === "/api/sync/upload" && request.method === "POST") {
          ...
          if (cl > 10 * 1024 * 1024) return json({ error: "データが大きすぎます" }, 413);
          let body;
          try {
            const text = await request.text();
            if (!text) return json({ error: "リクエストボディが空です" }, 400);
            body = JSON.parse(text);
          } catch(e) {
            return json({ error: "JSONの解析に失敗しました" }, 400);
          }
          await recordActivity(env, user.sub);
        ...
        if (path === "/api/sync/history" && request.method === "GET") {
          return handleSyncHistory(env, user);
        }
        if (path === "/api/sync/restore-version" && request.method === "POST") {
          let body;
          try {
            const text = await request.text();
            body = JSON.parse(text);
          ...
          return handleSyncRestoreVersion(body, env, user);
        }
        return json({ error: "Not found" }, 404);
      }
      ...
      if (path === "/api/test" && request.method === "GET") {
        return json({ ok: true, service: "Dayce API" });
      }
      ...
      if (path === "/api/analyze" && request.method === "POST") {
        ...
        if (userEmail) await recordActivity(env, userEmail);
        const body = await request.json();
        return handleAnalyze(body, env, userEmail, clientIP);
      }

      // --- Admin: アクティブユーザー統計 ---
      if (path === "/api/admin/active-users" && request.method === "GET") {
        const user = await getAuthUser(request, env);
        if (!user) return json({ error: "ログインが必要です" }, 401);
        return handleActiveUsers(env, user);
      }
      ...
      if (path === "/api/push/subscribe" && request.method === "POST") {
        ...
        const body = await request.json();
        if (!body.subscription || !body.subscription.endpoint) return json({ error: "Invalid subscription" }, 400);
        await env.SYNC_KV.put(`push:${user.sub}`, JSON.stringify(body.subscription));
        return json({ ok: true });
      }
      ...
      return json({ error: "サーバー内部エラーが発生しました" }, 500);
    }
  },

  // ===== Scheduled: 毎晩21:00 JST (12:00 UTC) にプッシュ通知送信 =====
  async scheduled(event, env, ctx) {
    ctx.waitUntil(sendDailyPushReminders(env));
  },
};
...
function concat(...bufs) {
  ...
  let off = 0;
  for (const b of bufs) { out.set(new Uint8Array(b instanceof ArrayBuffer ? b : b), off); off += b.byteLength; }
  return out;
}
...
async function webPushEncrypt(subscription, payloadStr) {
  const enc = new TextEncoder();
  ...
  const prk = await hkdf(auth, sharedSecret, concat(enc.encode('WebPush: info\0'), p256dh, ephPubRaw), 32);
  ...
  return concat(salt, new Uint8Array(rs), new Uint8Array([ephPubRaw.byteLength]), ephPubRaw, ciphertext);
}

async function buildVapidJwt(endpoint, privKeyB64, pubKeyB64) {
  const enc = new TextEncoder();
  const b64url = buf => btoa(String.fromCharCode(...new Uint8Array(buf))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
  const url = new URL(endpoint);
  const header = b64url(enc.encode(JSON.stringify({ alg:'ES256', typ:'JWT' })));
  const claims = b64url(enc.encode(JSON.stringify({ aud:`${url.protocol}//${url.host}`, exp: Math.floor(Date.now()/1000)+43200, sub:'mailto:support@dayce.app' })));
  const unsigned = `${header}.${claims}`;
  const privKey = await crypto.subtle.importKey('pkcs8', b64decode(privKeyB64), { name:'ECDSA', namedCurve:'P-256' }, false, ['sign']);
  const sig = await crypto.subtle.sign({ name:'ECDSA', hash:'SHA-256' }, privKey, enc.encode(unsigned));
  return `vapid t=${unsigned}.${b64url(sig)},k=${pubKeyB64}`;
}

async function sendPushNotification(subscription, payload, env) {
  if (!env.VAPID_PRIVATE_KEY || !env.VAPID_PUBLIC_KEY) { console.warn("VAPID keys not set"); return false; }
  try {
    const body = await webPushEncrypt(subscription, JSON.stringify(payload));
    const auth = await buildVapidJwt(subscription.endpoint, env.VAPID_PRIVATE_KEY, env.VAPID_PUBLIC_KEY);
...
async function sendDailyPushReminders(env) {
  const list = await env.SYNC_KV.list({ prefix: 'push:' });
  const payload = {
    title: '今日はどうでしたか？',
    body: 'Dayceに話しかけて、今日を振り返りましょう',
    url: 'https://dayce.app',
  };
  for (const key of list.keys) {
    const subStr = await env.SYNC_KV.get(key.name);
    if (!subStr) continue;
    try {
      const sub = JSON.parse(subStr);
      const result = await sendPushNotification(sub, payload, env);
      if (result === 'expired') {
        await env.SYNC_KV.delete(key.name);
        console.log(`Push subscription expired, removed: ${key.name}`);
      }
    } catch(e) {
      console.error(`Push failed for ${key.name}:`, e.message);
    }
  }
  console.log(`Daily push sent to ${list.keys.length} subscribers`);
}
