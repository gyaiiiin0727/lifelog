...
### セッション4で合意した方針
- **認証方式**: メール+パスワード認証がPWAに向いている。合言葉→メルパス→（必要なら後でGoogle追加）の順
...
### V. クイックボタンにページ移動ボタンを追加
**ファイル: `index.html`**
- 旧: セグメントボタン2つ（行動/お金の切替）
- 新: グリッドボタン5つ（3列レイアウト）
  - 上段: 🤖 AI相談, 🎯 目標, 📔 ジャーナル（ページ移動、青枠 `.quick-nav`）
  - 下段: ⏱️ 行動の記録, 💰 お金の記録（記録モーダル、グレー枠 `.quick-rec`）
